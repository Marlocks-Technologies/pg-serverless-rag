...
def _ascii_s3_metadata_value(value: object) -> str:
    """Encode values for S3 user metadata (HTTP headers must be US-ASCII per boto3)."""
    ...
    try:
        ...
        return "utf8-b64:" + base64.b64encode(text.encode("utf-8")).decode("ascii")


def _ascii_s3_metadata_key(key: object) -> str:
    k = str(key).lower().replace("_", "-")
    return k.encode("ascii", errors="replace").decode("ascii")
...
def upload_object(
    ...
    if metadata:
        # S3 user-defined metadata is sent as HTTP headers; keys and values must be ASCII.
        put_kwargs["Metadata"] = {
            _ascii_s3_metadata_key(k): _ascii_s3_metadata_value(v)
